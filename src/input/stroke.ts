...
export class StrokeEngine {
   ...
   *
   * `[TRAP]` **Never take this below 0.25.** `Reservoir.take` floors travel at
   ...
   * paint laid per cell jumps from 0.069 to 0.121 — 75 % more paint — while the
   * mark gets patchier, not smoother. Anyone reading that as "finer sampling
   * makes it worse" is reading a broken instrument.
   *
   * `[TRIED AND REVERTED, 2026-08-26]` A step that scaled with how broadside
   * the blade was — 0.90 edge-on, 0.30 across — on the theory that a blade
   * dragged across its own width beads, because its contact patch is thin in
   * the direction of travel. The theory holds and the effect measured
   * (broadside ripple 0.153 -> 0.118, edge-on untouched), but it cost about
   * 2.7x the footprint segments and the artist could not see the difference:
   * "it's no different, so just revert." Kept as a note because the mechanism
   * is real and may matter again if the footprint ever gets cheaper — but
   * whatever makes a broadside mark scale, most of it is NOT the sub-step.
   * docs/HANDOFF.md E12 has the numbers and the two instrument faults.
   */
  private maxStep = 0.9;
  ...
  get brushMix(): Float32Array<ArrayBuffer> {
    ...
    /* [TRAP, measured 2026-08-27 — do not "fix" this by sending the LOAD.]
     * The pickup pass scales its exchange by how unlike this vector is from the
     * cell it is over (docs/16 E10). It was tried both ways. Sending the tuft's
     * steady LOAD instead of what actually leaves the brush recycles paint: the
     * load of a yellow brush stays pure yellow, so a cell the brush has just
     * filled with its own picked-up blue reads as maximally UNLIKE and is
     * lifted at full rate, over and over. Lift fell to 12.5 % and the trail sat
     * flat at 93 % blue. The plumbing that carried the load to the shader was
     * removed with it; this vector is the whole answer.
     *
     * The question an exchange asks is "would trading change this cell?", and
     * that is about what is being DEPOSITED. */

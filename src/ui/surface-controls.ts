export interface SurfaceControlEvents {
  onLoadingChange?(loading: number): void;
...
export class SurfaceControls {
  ...
  constructor(private readonly root: HTMLElement, private readonly events: SurfaceControlEvents, initialEvapRate = 0) {
  ...
  private bind(initialEvapRate: number) {
    ...
    loading.addEventListener('input', () => {
      this.loading = parseFloat(loading.value);
      this.events.onLoadingChange?.(this.loading);
    });
    ...
    this.root.querySelector('#tilt-level')!.addEventListener('click', () => {
      puck.style.transform = 'translate(0, 0)';

...
/**
 * Makes the existing floating instrument panels behave like one window system.
 * The engine never knows this exists: it only moves DOM chrome around the canvas.
 */
export class WindowManager {
  ...
  constructor(workspace: HTMLElement = document.body) {
  ...
  private createChrome(element: HTMLElement, title: string) {
    ...
    titlebar.addEventListener('keydown', (event) => {
      if (event.key === 'Enter' || event.key === ' ') {
        ...
        element.classList.toggle('window-expanded');
      }
    });
  }
  ...
  private positionInitial(element: HTMLElement, side: WindowSpec['side']) {
    ...
    if (side === 'left') {
      element.style.left = `${Math.max(12, rect.left)}px`;
      element.style.right = 'auto';
    } else if (side === 'right') {
      element.style.right = `${Math.max(12, window.innerWidth - rect.right)}px`;
  ...
  private beginDrag(event: PointerEvent, host: HTMLElement, titlebar: HTMLElement) {
    ...
    host.classList.add('window-dragging');
    titlebar.classList.add('window-grabbing');
    host.setPointerCapture?.(event.pointerId);
    this.showSnapTargets(true);
    event.preventDefault();
  }
  ...
  private moveDrag(event: PointerEvent) {
    ...
    this.markSnapTargets(host);
  }

  private endDrag(event: PointerEvent) {
    if (!this.drag || event.pointerId !== this.drag.pointerId) return;
    const { host } = this.drag;
    const titlebar = host.querySelector<HTMLElement>('.window-titlebar');
    host.releasePointerCapture?.(event.pointerId);
    const snapped = this.snap(host);
    host.classList.remove('window-dragging');
    titlebar?.classList.remove('window-grabbing');
    this.drag = null;
    this.showSnapTargets(false);
    if (snapped) this.flashSnap(host);
    this.save();
  }
  ...
  private snap(host: HTMLElement) {
    const rect = host.getBoundingClientRect();
    let didSnap = false;
    if (rect.left <= SIDE_DISTANCE) {
      host.style.left = '12px';
      host.style.right = 'auto';
      didSnap = true;
    } else if (window.innerWidth - rect.right <= SIDE_DISTANCE) {
    ...
    for (const other of this.windows) {
      ...
      if (Math.abs(current.bottom - target.top) <= SNAP_DISTANCE && this.overlapX(current, target)) {
        host.style.top = `${target.bottom}px`;
        didSnap = true;
      } else if (Math.abs(current.top - target.bottom) <= SNAP_DISTANCE && this.overlapX(current, target)) {
        host.style.top = `${target.top - current.height}px`;
        didSnap = true;
      }
      if (Math.abs(current.right - target.left) <= SNAP_DISTANCE && this.overlapY(current, target)) {
        host.style.left = `${target.left - current.width}px`;
        didSnap = true;
      } else if (Math.abs(current.left - target.right) <= SNAP_DISTANCE && this.overlapY(current, target)) {
        host.style.left = `${target.right}px`;
        didSnap = true;
      }
    }
    return didSnap;
  ...
  private showSnapTargets(on: boolean) {
    this.snapLayer.classList.toggle('window-snap-active', on);
  }

  private markSnapTargets(host: HTMLElement) {
    const rect = host.getBoundingClientRect();
  ...
  private flashSnap(host: HTMLElement) {
    host.classList.add('window-snapped');
    window.setTimeout(() => host.classList.remove('window-snapped'), 420);
  }

  private clampAll() {
    for (const windowElement of this.windows) {
      const rect = windowElement.getBoundingClientRect();
      if (rect.right > window.innerWidth) windowElement.style.left = `${Math.max(12, window.innerWidth - rect.width - 12)}px`;
      if (rect.bottom > window.innerHeight) windowElement.style.top = `${Math.max(60, window.innerHeight - rect.height - 12)}px`;
    }
  }

  save() {
    const layout = this.windows.map((element) => {
      const rect = element.getBoundingClientRect();
      return { id: element.id, left: rect.left, top: rect.top, width: rect.width, height: rect.height };
    });
    localStorage.setItem('aniso-window-layout', JSON.stringify(layout));
  }
}

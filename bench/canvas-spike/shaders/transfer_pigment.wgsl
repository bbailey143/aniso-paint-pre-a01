// TransferPigment (§9, C97 Card 3). The exchange between pigment floating in
// the water and pigment adsorbed onto the paper. This distinction IS
// granulation and lifting — collapse the two and both behaviours disappear.
//
//   down = g * (1 - h*gamma) * rho
//   up   = d * (1 + (h-1)*gamma) * rho / omega
//
// Paper height enters through gamma, which is why heavy pigments settle into
// the hollows of rough paper. Nothing crosses the g/d boundary except through
// this pass, so the per-slot total (g + d) is conserved exactly.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (wet >= 0.5) {
        ...
        for (var k = 0; k < 8; k = k + 1) {
            ...
            // C97's clamps: neither side may be pushed past unity.
            if (d[k] + down > 1.0) { down = max(1.0 - d[k], 0.0); }
            if (g[k] + up   > 1.0) { up   = max(1.0 - g[k], 0.0); }
            // And neither side may go negative.
            down = min(down, g[k]);
            up   = min(up, d[k]);
